    ...
    return new Promise((resolve, reject) => {
        ...
        if (typeof require !== 'undefined') {
            try {
                ...
                console.error("Failed to load Node.js crypto modules:", error);
                reject(error);
            }
            return;
        }
        ...
        if (typeof window !== 'undefined') {
            ...
            if (naclLoaded && naclUtilLoaded) {
                ...
                // Fix the path to crypto.js - use the correct path
                script.src = './scripts/utils/crypto.js';
                script.onload = () => {
                    if (window.chainpad_crypto) {
                        if (window.nacl && !window.chainpad_crypto.Nacl) {
                            window.chainpad_crypto.Nacl = window.nacl;
                ...
                script.onerror = () => reject(new Error(`Failed to load crypto.js from ${script.src}`));
            ...
            loadScript('https://cdnjs.cloudflare.com/ajax/libs/tweetnacl/1.0.3/nacl-fast.min.js')
                .then(() => loadScript('https://cdn.jsdelivr.net/npm/tweetnacl-util@0.15.1/nacl-util.min.js'))
                .then(() => {
                    setupNaclUtil();
                    // Fix the path to crypto.js
                    return loadScript('./scripts/utils/crypto.js');
                })
                .then(() => {
                ...
                .then(resolve)
                .catch(error => {
                    console.error("Failed to load browser crypto modules:", error);
                    reject(error);
                });
        } else {
